...
function App() {
  ...
  /* =========================================================
     CORTACORRIENTE ACTIVADO: API DE SPOTIFY APAGADA TEMPORALMENTE
     ========================================================= */
  /*
  useEffect(() => {
    ...
    if (spotifyCode) {
        ...
        .then(datos => {
            if (datos.access_token) {
                ...
                .then(data => {
                  if (data.error) {
                    console.error("Error de Spotify:", data.error.message);
                    return;
                  }
  ...
  */
  ...
  const handleLogin = () => {
    ...
    // Te damos acceso directo
    setUsuario("Usuario de Pruebas");
    setIdEchohead(1);
    setSesionIniciada(true);
  };
  ...
  if (sesionIniciada) {
    ...
    <>
      ...
      {/* El reproductor se renderiza afuera de las rutas para que siempre esté visible */}
      <Reproductor />
    </>
    );
  }
